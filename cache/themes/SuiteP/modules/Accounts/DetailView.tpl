...
<span class="sugar_field" id="{$fields.gender_c.name}">
...
{capture name="label" assign="label"}{sugar_translate label='LBL_TEST_FIELD' module='Accounts'}{/capture}
{$label|strip_semicolon}:
</div>
...
{if !$fields.test_field.hidden}
{counter name="panelFieldCount" print=false}

{if strlen($fields.test_field.value) <= 0}
{assign var="value" value=$fields.test_field.default_value }
{else}
{assign var="value" value=$fields.test_field.value }
{/if} 
<span class="sugar_field" id="{$fields.test_field.name}">{$fields.test_field.value}</span>
{/if}
